...
func postgresConfigWithCredentials(user, userFile, pass, passFile string) *IpMatchDatabaseConfig {
	return &IpMatchDatabaseConfig{
...
func TestCredentialFiles(t *testing.T) {
	...
	t.Run("redis credentials are optional but checked when set", func(t *testing.T) {
		cfg := &IpMatchDatabaseConfig{Database: DatabaseConfig{Type: "redis", Redis: &RedisConfig{
...
func TestResolvePathsUsesBaseDirectory(t *testing.T) {
	cfg := &IpMatchDatabaseConfig{Database: DatabaseConfig{
		Type: "postgres",
		Postgres: &PostgresConfig{
