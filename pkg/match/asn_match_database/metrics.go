...
func (c *asnMatchDatabaseController) observeRequest(authority, result string) {
	requestsTotal.WithLabelValues(authority, c.name, c.dbType, result).Inc()
}

// observeQuery records a database query
func (c *asnMatchDatabaseController) observeQuery(authority, dbType, result string, duration time.Duration) {
...
func (c *asnMatchDatabaseController) observeUnavailable(authority, dbType string) {
