...
func NewSQLServerDataSource(ctx context.Context, config *SQLServerConfig) (*SQLServerDataSource, error) {
	...
	if config.Pool != nil {
		...
		if config.Pool.MaxIdleConnections >= 0 {
			if config.Pool.MaxIdleConnections > maxOpenConns {
...
// Contains checks if the ASN exists in SQL Server.
func (s *SQLServerDataSource) Contains(ctx context.Context, asn uint) (bool, error) {
	rows, err := s.db.QueryContext(ctx, s.query, int64(asn))
	if err != nil {
